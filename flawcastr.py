                ...
                if (
                    ...
                    if hasattr(config_module, var_name):
                        ...
                        try:
                            if attr_type is int:
                                ...
                                if "." in value:
                                    ...
                                    if float_value.is_integer():
                                        setattr(
                                            config_module, var_name, int(float_value)
                                        )
                                    else:
                                        print(
                                            f"Value {value} for variable '{var_name}' is not a clean integer."
                                        )
                                else:
                                ...
                                try:
                                    ...
                                    print(
                                        f"Date conversion error for variable '{var_name}': {e}. Check the date format in your CSV."
                                    )
                            else:
                            ...
                            print(
                                f"Value conversion error for variable '{var_name}': {e}"
                            )
    return loaded_vars
...
class ClientInfoDialog(QDialog):
    ...
    def get_data(self):
        return {
            ...
            "client2_age": (
                int(self.client2_age.text()) if self.client2_age.text() else 0
            ),
...
def check_expiry():
    ...
    if current_date > expiry_date:
        ...
        msg.setText(
            f"Sorry - this version of Flawcastr stopped operating on {formatted_date}."
        )
...
def apply_initial_window_data_to_config(client_info):
    ...
    config.education_start_year_child1 = (
        config.age_of_providing_initial_educational_assistance - config.child1_age
    )
    config.education_start_year_child2 = (
        config.age_of_providing_initial_educational_assistance - config.child2_age
    )
    config.education_start_year_child3 = (
        config.age_of_providing_initial_educational_assistance - config.child3_age
    )
    config.education_start_year_child4 = (
        config.age_of_providing_initial_educational_assistance - config.child4_age
    )
    config.education_start_year_child5 = (
        config.age_of_providing_initial_educational_assistance - config.child5_age
    )
...
if __name__ == "__main__":
    ...
    if dialog.exec_():
        ...
        config.individual_or_couple = client_info["individual_or_couple"]
        config.client1_name = client_info["client1_name"]
        config.client1_age = client_info[
            "client1_age"
        ]  # This is now an integer from get_data()
        config.client2_name = client_info["client2_name"]
        config.client2_age = client_info[
            "client2_age"
        ]  # This is now an integer from get_data()
        config.age_to_follow_to = client_info[
            "age_to_follow_to"
        ]  # This is now an integer from get_data()
